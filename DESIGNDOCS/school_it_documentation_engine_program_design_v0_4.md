...
| 33 | Finding context (annotation) is preferred over finding suppression | v0.5.3 | Findings can never be hidden after generation — they can only be annotated with a context note; preserves report integrity while allowing reviewer to communicate remediation status; planned for pre-v1.0 implementation |
| 34 | Answer amendment log is planned pre-v1.0 using a separate answer_history table | v0.5.3 | On every save where raw_answer or answer_status changes, the previous record is written to answer_history; no practical DB bloat risk; displayed in report appendix |
