...
| 30 | Section progress bar is live (JS-driven) and updates on every interaction without a server round-trip | v0.5.3 | Bar shows answered / unknown / skipped counts and changes colour (blue → amber → green) as completion increases; hooks into existing markAnswered, updateCard, toggleSkip, toggleUnknown, and evaluateAllConditions functions |
| 31 | Unknown answers are surfaced in a dedicated summary panel rather than only in section-level stats | v0.5.3 | Panel on Assessment Summary page lists every unknown by section, with question ID, prompt, and a direct link back to the section; only shown when unknowns exist; makes the gap visible at a glance |
| 32 | Session export format is versioned JSON (export_format: school_it_engine_session_v1) | v0.5.3 | Includes school profile, session metadata, and full answers dict; import route validates format tag and rejects duplicate session_ids without modifying existing data; school profile is only restored if none is currently set |
| 33 | Finding context (annotation) is preferred over finding suppression | v0.5.3 | Findings can never be hidden after generation — they can only be annotated with a context note; preserves report integrity while allowing reviewer to communicate remediation status |
| 34 | Answer amendment log records revisions to completed sections only | v0.5.3 | The `answer_history` table captures the previous raw_answer and answer_status before any save that overwrites a record in a section already marked complete; first-pass saves are not tracked; no meaningful DB bloat risk |
| 35 | Home page uses a two-tab layout separating "New Assessment" from "Saved Assessments" | v0.5.3.2 | Tabs prevent the module cards and session list from competing for attention; selected tab persists via localStorage; default tab is Saved when sessions exist, New when none do |
| 36 | Session Archive and Delete are moved to a dedicated /session/\<id\>/manage page | v0.5.3.2 | Removes destructive actions from the home page session card row; manage page provides clear descriptions and a two-step inline confirmation for Delete; Export is also surfaced there |
| 37 | Sessions are identified by a human-readable label (#N · Started YYYY-MM-DD) rather than a truncated UUID | v0.5.3.2 | UUIDs are internal infrastructure with no user value; human labels are stable, numbered in creation order, and comprehensible at a glance |
| 38 | Privacy banner is shown in full only on Home and Setup pages; all other pages show a compact nav indicator | v0.5.3.2 | Banner blindness reduces the effectiveness of a permanent banner; Home and Setup are the pages where a new user most needs reassurance |
| 39 | Archived sessions are surfaced in a dedicated third tab on the home page rather than hidden entirely | v0.6.0 | Previously deprecated sessions were invisible in the UI; the Archived tab shows them alongside Restore and manage actions; this closes the gap between "hidden but kept" and "visible and manageable" without cluttering the active session list |
| 40 | Finding context notes are gated on last_exported — the "Add note" UI appears only after the first DOCX download | v0.6.0 | Prevents annotation before the reviewer has seen the report; ensures context notes are meaningful responses to the printed findings rather than pre-emptive edits; the gate is the last_exported timestamp already tracked in assessment_session |
| 41 | Draft DOCX treatment uses a cover-page marker plus an exec summary callout listing specific incomplete sections, not a per-section watermark | v0.6.0 | A per-section watermark would require identifying which sections have findings but no completed data — difficult to define cleanly; a cover marker plus a single named list in the exec summary gives the reader full information without repeating the warning throughout the document |
| 42 | Module 2 phased timeline start date is optional — blank skips the timeline section entirely | v0.6.0 | The previous implementation required a date and the workaround ("leave today's date to skip") was both misleading and wrong — today's date still generated a timeline; making the field optional with clear copy is the correct UX |
...
| 5 | What are the DOCX section mapping conventions for module report output? | **Closed** — conventions emerged organically during the Module 1 build; each section maps to a Heading 2 block, findings render as bordered paragraphs with severity-coloured left borders, actions in pale-green callout boxes, and healthy sections get a ✓ marker. Module 2 follows the same conventions. |
...
The following items are the most valuable candidates for the next development cycle, in priority order:

1. **Whole-school export / import** — single-action backup and migration of all sessions and the school profile to a new machine. Low implementation effort (S scope), high practical value for any school that wants to archive or transfer their data. See Future Ideas for the full spec.
2. **Rules-in-YAML architecture** — move scoring and finding logic out of Python and into declarative YAML, making it possible to add new modules without writing a new rules engine. M+ to L scope. Recommend Module 1 first; Module 2 follows the same evaluator once it's working.
3. **Re-assessment tracking** — store multiple snapshots per school and surface score deltas across sessions. Requires a snapshot model (assessment_snapshot table keyed by session_id + snapshot date) and a delta view on the Summary page.
4. **Executive / board report variant** — a shorter, less technical DOCX variant of the Module 1 report focused on key risks, overall score, and Phase 1 summary. Low-to-medium implementation effort once the full report is stable.
5. **PDF export** — generate a PDF alongside or instead of DOCX. Approach not yet decided (LibreOffice headless vs. pure Python renderer vs. WeasyPrint). Evaluate options before committing.

*Previously listed goals (score display format, DOCX section mapping, first real assessment tuning) are now closed — see Open Questions above.*
