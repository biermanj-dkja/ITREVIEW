...
**Unknown override:** Critical questions: 7.2, 7.8, 7.12, 7.13. Unknown on any raises floor to concern.
...
**Conditions:**
- 3.6 (firewall exists) is `no` or `unknown`
...
**Pattern:** Graduated Severity + Cross-Section Reference
**Trigger questions:** 3.17, 3.6

**Conditions:**
- 3.6 indicates a firewall exists
...
**Pattern:** Graduated Severity
**Trigger question:** 3.8

**Conditions:**
- Wireless APs exist
- AND 3.8 (APs inventoried with model and firmware) is `partial`, `no`, or `unknown`
...
- AND 3.18 (config backup) is `no` or `unknown`
...
| R3-004 fires (no firewall or unknown) | urgent |
...
| Only watch-level findings fire | watch |
| No findings fire | healthy |
| More than one-third of questions unknown | watch minimum; escalates to concern if 3.6, 3.12, 3.15, or 3.18 are among the unknowns |
...
If any of the following are unknown, section floor rises to **concern**:
- 8.2 (endpoint protection deployed)
...
**Pattern:** Graduated Severity + Cross-Section Reference
**Trigger questions:** 8.2, 1.14, 5.3

**Conditions:**
- Endpoints are in scope
- AND 8.2 (endpoint protection deployed on supported devices) is `yes some`, `no`, or `unknown`
...
**Escalation modifier:** If 8.2 is `no` or `unknown` AND 5.3 is `yes all relevant grades` (1:1 student program), elevate finding prominence — unprotected student devices at scale is both a security and safeguarding issue that should be noted explicitly.
...
**Escalation modifier:** If 8.11 is `yes` AND 8.2 is `no` or `unknown` (no endpoint protection), escalate to **urgent** — active security concerns in an unprotected environment represent compounded risk.
...
**Conditions (all must be true):**
- 8.2 (endpoint protection) is `no` or `unknown`
...
| F8-001 fires at urgent (no endpoint protection) | urgent |
...
| Only watch-level findings fire | watch |
| No findings fire | healthy |
| More than one-third of questions unknown | watch minimum; escalates to concern if 8.2, 8.5, or 8.8 are among the unknowns |
