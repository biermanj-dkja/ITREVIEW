...
# v0.2 Addendum — Updates as of v0.7.1

This addendum documents changes to scoring behaviour and implementation decisions made after
v0.1 was written. The section weight table and individual question point values in v0.1 remain
unchanged. This addendum records what was added, what was implemented, and what gaps remain.

---

## Overall Weighted Score — Implementation Status

The total score is now included in the Module 1 DOCX report executive summary (added v0.7.1).
It appears as a coloured score box showing the numeric percentage, severity band, and a
two-sentence explanation of the weighting method.

**What is implemented:**
- Weighted average of Sections 2–9 using the weights from the section weight table above
- Severity band derived from the total score interpretation table
- Short plain-language explanation of what is and is not included

**What is not yet implemented (still deferred):**
- Section-by-section contribution table in the report (the breakdown table shown in the
  "Score Display Specification" example above — showing each section's weight and contribution
  percentage — is not yet in the DOCX)
- Question-level score display in the UI (point values are shown per question as designed,
  but earned/maximum is not shown inline during answering)

---

## Escalation Rules Added in v0.2 Rule Schema

The following escalation and composite behaviours were defined in the rule schema after this
scoring weights document was written. They do not change point values but clarify how the
engine uses scores in combination.

### Section 5: Endpoints — escalation for 1:1 environments
- R5-003 escalates from concern to urgent when MDM is absent AND the school has a 1:1 student
  device program. The escalation is driven by question 5.15 (1:1 program confirmed) combined
  with 5.5 (MDM absent). Point value for 5.5 remains 4pts; the severity escalation is a rule
  behaviour, not a scoring change.

### Section 6: Server documentation — amplification modifier
- Question 6.15 (server purpose documented) carries 3pts and acts as a severity amplifier for
  6.14 findings. When 6.15 is unknown/no and 6.14 also fires, the combined finding escalates.
  This is a cross-question interaction, not a change to individual point values.

### Section 8: Composite F8-C01
- F8-C01 fires when both MFA gaps (F4-C01 or F4-005) and endpoint protection gaps (F8-001)
  are present simultaneously. This composite is a report assembly rule and does not change
  how individual questions are scored.

### Section 9: RA-011 amplification
- When F9-C01 or F9-005 fires (no documentation / environment non-transferable), the report
  assembler adds an amplification sentence to findings in Sections 3, 6, and 7 that reference
  missing documentation. This is a report narrative modifier only — it does not change scores
  or finding severities. The amplification behaviour is implemented in `rules_engine.py` via
  `apply_amplification()`.

---

## Known Scoring Gaps as of v0.7.1

| # | Gap | Notes |
|---|-----|-------|
| 1 | Section-by-section contribution table not in DOCX | The score box shows overall %; the per-section breakdown table is not rendered |
| 2 | Question-level earned/max not shown inline during answering | Point values are shown; earned scores are not shown until section complete |
| 3 | Unknown answer floor enforcement varies by section | The global floor rule (any unknown → watch minimum) is implemented; per-section critical-unknown overrides are defined in the rule schema but implementation consistency has not been audited |
| 4 | Score display format note for Section 9 (low-weight explanation) | The plain-language note is defined in this document but is not yet rendered in the DOCX section score display |

---

## Composite Finding Scoring Clarification

Composite findings (`F2-C01`, `F4-C01`, `F5-C02`, `F6-C01`, `F7-C01`, `F8-C01`, `F9-C01`)
do not carry their own point weight. They fire based on the component questions already being
evaluated. When a composite fires and suppresses its component findings, the underlying scores
are not changed — suppression is a report display decision only, not a scoring adjustment.

The count of "findings absorbed into composites" displayed in the Assessment Overview reflects
how many component findings were suppressed in favour of a composite. As of v0.7.1, this
mechanism is implemented in `apply_suppression()` in `rules_engine.py` but the count may
read 0 for assessments where composite trigger conditions are not fully met. A diagnostic
trace is planned before the next release.

---

*v0.2 addendum written v0.7.1. The section weight table and question point values in v0.1
remain the canonical reference and have not changed.*
